describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let i = 0; i < prodItemsData.length; i++) {
      const title = prodItemsData[i].title;
      const price = prodItemsData[i].price;
      const image = prodItemsData[i].image;
      if (!title || !price || !image) allArePopulated = false;
    }
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    console.log('Checking the "Remove from Cart" button...');
    buttonText = await button.getProperty('innerText');
    text = await buttonText.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let item of prodItems) {
      ...
      if (innerText !== 'Remove from Cart') allInCart = false;
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let item of prodItems) {
      ...
      const button = await shadowRoot.$('button');
      const innerText = await (await button.getProperty('innerText')).jsonValue();
      if (innerText !== 'Add to Cart') allNotInCart = false;
    }
